<div class="fixed pin-t pin-x z-40">
    ...
    <nav class="flex items-center justify-between text-black bg-navbar shadow-xs h-16">
        ...
        <div class="block mx-4 flex items-center">
            ...
            {{-- Dark mode toggle --}}
            <button id="dark-toggle" onclick="larecipeToggleDark()" title="Toggle dark mode" aria-label="Toggle dark mode">
            ...
            @auth
                ...
                <larecipe-dropdown>
                    <larecipe-button type="white" class="ml-2">
                        {{ auth()->user()->name ?? __('Account') }} <i class="fa fa-angle-down"></i>
                    </larecipe-button>
                    ...
                    <template slot="list">
                        <form action="/logout" method="POST">
                            ...
                            <button type="submit" class="py-2 px-4 text-white bg-danger inline-flex"><i class="fa fa-power-off mr-2"></i> {{ __('Logout') }}</button>
                        </form>
                    </template>
                </larecipe-dropdown>
                {{-- /account --}}
